...
export default function DemandForecasting() {
    ...
    const visibleForecasts =
        filter === 'All'
            ? forecasts
            : forecasts.filter((forecast) => forecast.level === filter);
                ...
                <main className="p-4 lg:p-7">
                    <div className="mx-auto max-w-7xl space-y-5">
                        <div className="grid gap-2 rounded-2xl border border-[#d0e5da] bg-white/80 p-2 shadow-sm sm:grid-cols-3">
                            {['Cold Season', 'Hot Season', 'Rainy Season'].map(
                                (option) => (
                                    <button
                                        key={option}
                                        type="button"
                                        onClick={() => setSeason(option)}
                                        className={`rounded-xl px-4 py-2.5 text-sm font-semibold transition ${season === option ? 'bg-[#004d38] text-white shadow-sm' : 'text-slate-700 hover:bg-emerald-50'}`}
                                    >
                                        {option}
                                    </button>
                                ),
                        ...
                        <section className="rounded-2xl border border-emerald-200 bg-emerald-50/80 p-4">
                            <div className="flex flex-wrap items-start gap-3">
                                ...
                                <div>
                                    <h2 className="text-xs font-bold text-green-900">
                                        {season} Focus
                                    </h2>
                                    <p className="mt-1 text-xs text-green-900/80">
                                        High incidence of Asthma Exacerbation,
                                        Cough &amp; Colds, and URTI.
                                    </p>
                                    <div className="mt-3 flex flex-wrap gap-2">
                                        {[
                                            ...
                                            <span
                                                key={tag}
                                                className="rounded-full bg-emerald-100 px-2.5 py-1 text-[10px] font-semibold text-emerald-800"
                                            >
                                                {tag}
                                            </span>
                                        ))}
                        ...
                        <section className="rounded-2xl border border-[#d0e5da] bg-white shadow-sm">
                            <div className="flex flex-col gap-3 border-b border-slate-100 p-4 sm:flex-row sm:items-center sm:justify-between">
                                <div>
                                    ...
                                    <p className="mt-1 text-[11px] text-slate-500">
                                        Projected walk-ins by barangay for the
                                        next 30 days
                                    </p>
                                </div>
                                <span className="rounded-md bg-emerald-50 px-3 py-1.5 text-[11px] font-semibold text-emerald-700 ring-1 ring-emerald-200">
                                    Linear Model · R²: 0.91 · MAE: ±42
                                </span>
                            </div>
                            ...
                            <div className="flex flex-wrap gap-2 border-b border-slate-100 px-4 py-3">
                                {(
                                    ...
                                    <button
                                        ...
                                        className={`rounded-full px-3 py-1 text-[10px] font-bold ring-1 transition ${filter === option ? 'bg-[#003b30] text-white ring-[#003b30]' : `${option === 'All' ? 'bg-slate-50 text-slate-600 ring-slate-200' : levelStyles[option]} hover:opacity-80`}`}
                                    >
                                        {option}
                                    </button>
                                ))}
                            ...
                            <div className="grid gap-3 p-4 sm:grid-cols-2 xl:grid-cols-3">
                                {visibleForecasts.map((forecast) => (
                                    <article
                                        key={forecast.name}
                                        className={`rounded-xl border bg-white p-3 shadow-sm ring-1 ${forecast.level === 'Critical' ? 'border-red-200 ring-red-100' : forecast.level === 'High' ? 'border-orange-200 ring-orange-100' : forecast.level === 'Moderate' ? 'border-yellow-200 ring-yellow-100' : 'border-green-200 ring-green-100'}`}
                                    >
                                        <div className="flex items-start justify-between gap-2">
                                            ...
                                            <span
                                                className={`rounded-full px-2 py-1 text-[9px] font-bold uppercase ${levelStyles[forecast.level]}`}
                                            >
                                                {forecast.level}
                                            </span>
                                        </div>
                                        <p className="mt-4 font-mono text-2xl font-bold text-[#003b30]">
                                            ~{forecast.demand}
                                        </p>
                                        ...
                                        <button
                                            type="button"
                                            className="mt-3 border-t border-slate-100 pt-2 text-[10px] font-semibold text-emerald-700 hover:text-emerald-900"
                                        >
                                            View Details →
                                        </button>
                                    </article>
